...
func (room *ChatRoom) launchServer(){
	...
	for {
		...
		outgoings := room.acceptData(<-conns)
		room.echoData(<-outgoings)
		close(outgoings)
	}
}
...
func (room *ChatRoom) acceptData(client net.Conn){ // return outs?
	fmt.Print("\n")
...
func (room *ChatRoom) echoData(in <-chan string){
	message := <- in
